        ...
        <section className={classes.container}>
            {
                props.groups.map(group=>{
                    ...
                    return <article className={classes.groupCategory} key={id}>
                        <h4 className={classes.groupTitle}><a style={{fontSize:'2.5rem' , color:'black'}} className={classes.groupCategoryLink} href={`/products/group/${id}/${name.trim().replaceAll(' ', '-')}`}>{group.group}</a></h4>
                        {products.map(prod=>{
